...
public class CustomActions {
    ...
    public void setInitialDrivePosition(String preloadType, String cycleType){
        ...
        if (preloadType.equals("specimen") && cycleType.equals("sample")) {
            drive = new PinpointDrive(hardwareMap, new Pose2d(-hw.robotDimensions.width/2, -70+(hw.robotDimensions.length/2), Math.toRadians(270)));
        }
        ...
        // Barely outside of net zone
            ...
            drive = new PinpointDrive(hardwareMap, new Pose2d(-32.125-(hw.robotDimensions.length/2), -70+(hw.robotDimensions.width/2), Math.toRadians(180)));
        }
        // Right side of robot is touching field perimeter, Outtake side is on the left side of seam that is on the edge of field tile
        // Barely outside of net zone
    ...
    public Pose2d getInitialDrivePosition(String preloadType, String cycleType){
        if (preloadType.equals("specimen") && cycleType.equals("sample")) {
            return new Pose2d(-hw.robotDimensions.width/2, -70+(hw.robotDimensions.length/2), Math.toRadians(270));
        }

        else if (preloadType.equals("sample") && cycleType.equals("sample")) {
            return new Pose2d(-32.125-(hw.robotDimensions.length/2), -70+(hw.robotDimensions.width/2), Math.toRadians(180));
        }
